...
def extract_bronchial(ct_slice, xy_spacing=1.):
    """
    Detection the brextract_bronchialonchi and the trachea on CT.
...
def grow_lungs(patient, seeds):
    ...
    new_ = np.subtract(lungs_seeds, scipy.ndimage.morphology.binary_opening(seeds), dtype=np.float32)
    return (scipy.ndimage.morphology.binary_opening(new_))
...
def conventional_lung_segmentation(patient):
    ...
    flip_lung, glungs_postprocessingrow_lungs, lungs_postprocessing.
...
def costal_surface(lung, z_coords, max_coor, combined):
    ...
    for coord in np.unique(z_coords):
        ...
        erroneus[coord] =  np.subtract(skimage.morphology.convex_hull_object(roi), roi, dtype=np.float32)
